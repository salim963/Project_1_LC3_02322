...
isPrime      LD R5, MULTI    ; give number 9 to R5
...
loops        LDR R2,R1,#0          ; get the value from R1 
             BRz NOtprime          ; check if the number in R2 is equel to 0, if it is not jamp to NOtprime
            
             NOT R2,R2            ; make the value to negativ value
             ADD R2,R2,#1         ; add 1 to R2
             ADD R3,R2,R0         ; put R2 and R0 in R3
             BRz myPrime          ; if =0 soskip over to myPrime, if not continu the code 
